...
export function CoverflowCarousel({
  ...
  if (count === 0) {
    return (
      <div className="flex h-64 items-center justify-center rounded-[var(--radius-lg)] border border-dashed border-border text-sm text-muted-foreground">
        No images to show in carousel view.
      </div>
    );
  }
    ...
    >
      <div className="relative">
        <div
          ...
          onDoubleClick={() => active && onSelect?.(active, selected)}
          onKeyDown={(event) => {
            if (event.key === "ArrowLeft") {
              ...
              active && onSelect?.(active, selected);
            }
          }}
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background hover:scale-105"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background hover:scale-105"
            >
